...
import { parseDir, CLAUDE_CODE_FIELDS, type GenericEntry } from '@ebowwa/jsonl-hft';

// Type alias for parsed entries
type ParsedEntry = GenericEntry & {
  uuid: string;
  session_id: string;
  timestamp: string;
  role: string;
...
export class ClaudeCodeHistoryService {
  ...
  private async listSessionsFast(
    ...
    const parsedEntries = parseDir(projectDirPath, CLAUDE_CODE_FIELDS) as ParsedEntry[];
